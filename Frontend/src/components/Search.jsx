...
const Search = () => {
    ...
    try {
      ...
      if (searchValue) {
        const filtered = criminalList.filter((criminal) =>
          criminal.name.toLowerCase().includes(searchValue)
        );
  ...
  const handleClipboard = () => {
    navigator.clipboard.writeText(selectedCriminal.dna);
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {searchList.map((criminal) => (
            <div key={criminal.id} className='bg-gray-800 text-white p-6 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105'>
              ...
              <button
                className='block w-full bg-green-500 p-3 rounded-md text-white font-bold hover:bg-green-600 transition-all duration-300'
      ...
      {showModal && (
        ...
        >
          <div className='bg-gray-900 text-white p-10 rounded-2xl relative max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl'>
            <button
              className='absolute top-4 right-4 w-10 text-white bg-red-500 rounded-full p-2 hover:bg-red-600 transition-all duration-300'
            ...
            <div className='flex flex-col lg:flex-row items-start gap-6'>
              <div className='flex-1'>
                ...
                <p className='mt-4'><strong>Age:</strong> {selectedCriminal.age}</p>
                <p className='mt-4'><strong>Address:</strong> {selectedCriminal.address}</p>
                <p className='mt-4'><strong>Date Of Birth:</strong> {selectedCriminal.dob.slice(0, 10)}</p>
                <p className='mt-4'><strong>Contact:</strong> {selectedCriminal.contact}</p>
                <p className='mt-4'><strong>Arrested On:</strong> {selectedCriminal.arrestedOn.slice(0, 10)}</p>
                <p className='mt-4'><strong>Arrested By:</strong> {selectedCriminal.arrestedBy}</p>
                <p className='mt-4'><strong>Location:</strong> {selectedCriminal.location}</p>
                <p className='mt-4'><strong>Crime:</strong> {selectedCriminal.crime}</p>
                <p className='mt-4'><strong>State:</strong> {selectedCriminal.state}</p>
                <p className='mt-4'><strong>Height:</strong> {selectedCriminal.height}</p>
                <p className='mt-4'><strong>Weight:</strong> {selectedCriminal.weight}</p>
                <p className='mt-4'><strong>Complexion:</strong> {selectedCriminal.complexion}</p>
                <p className='mt-4'><strong>Blood Group:</strong> {selectedCriminal.bloodGroup}</p>
                <p className='mt-4'><strong>Prison Name:</strong> {selectedCriminal.prisonName}</p>
                <div className='flex items-center'>
                  <p className='mt-4 text-ellipsis overflow-hidden whitespace-nowrap w-56'>
                    <strong>DNA Sequence:</strong> {selectedCriminal.dna}
                  </p>
                  <i class='fa-solid fa-clipboard mt-3 ml-3 hover:text-gray-400 cursor-pointer' onClick={() => handleClipboard()}></i>
                </div>

                <p className='mt-4'><strong>Station Name:</strong> {selectedCriminal.stationName}</p>
                <p className='mt-4'><strong>Comments:</strong> {selectedCriminal.comments || 'none'}</p>
                <div className='mt-4'>
